...
LLM한테 일을 시키면 생산성은 분명 오릅니다. 그런데 내가 만들어둔 규칙·에이전트·스킬이 정말 잘 쓰이는지는 알 길이 없었습니다. 그래서 자산이 얼마나 효율적으로 쓰이는지 재서 눈으로 보려고 만들었습니다. 안 쓰는 건 지울 후보고, 자주 쓰는 건 더 다듬을 거리입니다. 고칠 거리를 찾아 반영하면 다음엔 손이 덜 가고요.
...
`npm run` 대신 컨테이너로 상시(재부팅 생존) 가동합니다.

```bash
cp .env.example .env          # CONNECTLY_PATH 등 호스트 경로를 자기 환경에 맞게 수정
lsof -ti:3001 -ti:5173 | xargs kill 2>/dev/null || true   # 수동 기동 중이면 종료(포트 충돌)
docker compose up -d --build
```

- 서버 `:3001`(MCP `http://localhost:3001/mcp`) · 대시보드 `:5173`.
- DB는 named volume `opspilot-data`(영속). 초기화는 `docker compose down -v`.
- 컨테이너는 `node --import tsx`로 서버를 PID 1 기동 → `docker stop` 정상 종료. `restart: unless-stopped`로 재부팅 후 자동 가동.
- 첫 기동 후 프로젝트를 **컨테이너가 보는 경로**(예: `${CONNECTLY_PATH}` 마운트 지점 `/srv/connectly`)로 등록(linked)하면 apply→git이 그 마운트(=호스트 레포)에 커밋합니다.
- macOS 기준 컨테이너는 root로 실행(named volume 쓰기 + Docker Desktop bind 매핑). Linux에서 bind-mount 소유권 정합이 필요하면 compose에 `user:`와 볼륨 chown을 추가하세요.
