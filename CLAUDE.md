...
- **MCP 어댑터**: `:3001/mcp` 에 7개 툴(`scan_project`/`list_projects`/`list_assets`/`list_scenarios`/`start_run`/`get_run`/`compare_runs`). domains 함수 재사용. 등록: `claude mcp add --transport http opspilot http://localhost:3001/mcp`. 데이몬 로그: `mcp/log.ts` (`OPS_TERM_LOG=off`).
