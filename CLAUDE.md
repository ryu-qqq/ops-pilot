...
- **공통** agent/skill 수정 → [agent-crew](https://github.com/ryu-qqq/agent-crew) 레포에서 버전(tag) 올린 뒤 소비 프로젝트에 `sync_agent_crew` MCP 또는 `POST /projects/:id/sync-agent-crew`.
...
- **MCP 어댑터**: `:3001/mcp` 에 12개 툴(`scan_project`/`sync_agent_crew`/`list_projects`/…/`review_proposals`). domains 함수 재사용. 등록: `claude mcp add --transport http opspilot http://localhost:3001/mcp`. 데이몬 로그: `mcp/log.ts` (`OPS_TERM_LOG=off`).
