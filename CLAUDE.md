...
- **MCP 어댑터(OPSP-18)**: `:3001/mcp` 에 7개 툴(`scan_project`/`list_projects`/`list_assets`/`list_scenarios`/`start_run`/`get_run`/`compare_runs`) 노출. domains 함수 그대로 재사용(REST 와 병렬 입구, 로직 중복 X). Streamable HTTP transport, stateless. 등록: `claude mcp add --transport http opspilot http://localhost:3001/mcp`. 데이몬 pane 컬러 한 줄 로그는 `mcp/log.ts`(pino 분리, `OPS_TERM_LOG=off` 로 끔, Fastify logger 기본 level=warn).
