...
> **구현 상태(2026-06-03, merge `d9f99ef`)**: Follow-up 1~4 구현 완료. `feedback/auto-ingest.ts`
> `runAutoIngestScan` + `plugins/auto-ingest-scan.ts`(autoload) + `ingest_bundle.ingest_trigger`
> 컬럼(auto|manual). **off-by-default** — `OPS_AUTO_INGEST=1` 일 때만 활성(LLM 토큰 비용).
> 자가 루프 차단(ops() 커밋 제외)·merge 제외·BATCH/WINDOW 레이트 제한. eval→review 연쇄·
> apply HITL 불변. **5·6은 미구현(범위 밖)** — 사람 소표본 외부 신호(5), 자동 apply·거절 학습(6).
